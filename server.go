...
func parseParams(c *gin.Context) map[string]interface{} {
    ...
    // NEW PARAMETERS
    params["drop_threshold"], _ = strconv.Atoi(c.DefaultPostForm("drop_threshold", "50"))

    // total_sequence_scoring is a bool checkbox; parse "true"/"false"
    totalSeqScoringStr := c.DefaultPostForm("total_sequence_scoring", "false")
    totalSeqBool, _ := strconv.ParseBool(totalSeqScoringStr)
    params["total_sequence_scoring"] = totalSeqBool
...
func prepareCommand(params map[string]interface{}, filePath string) *exec.Cmd {
    ...
    for i, score := range consecutiveATScoring {
        ...
        atScoringStr.WriteString(fmt.Sprintf("%.1f", score))
    }

    cmdArgs := []string{
        "--fasta", filePath,
        "--GC_weight", fmt.Sprintf("%.2f", params["GC_weight"]),
        "--AT_weight", fmt.Sprintf("%.2f", params["AT_weight"]),
        "--GT_weight", fmt.Sprintf("%.2f", params["GT_weight"]),
        "--AC_weight", fmt.Sprintf("%.2f", params["AC_weight"]),
        ...
        "--drop_threshold", fmt.Sprintf("%d", params["drop_threshold"]),
    }

    // If total_sequence_scoring is true, add the flag
    if params["total_sequence_scoring"].(bool) {
        cmdArgs = append(cmdArgs, "--total_sequence_scoring")
    }
